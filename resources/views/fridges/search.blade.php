    ...
    <div class="flex flex-col m-auto w-3/4 h-full bg-white p-6 rounded-lg">
        <div class="grid sm:grid-cols-2 md:grid-cols-4 gap-6 mx-6 bg-white">
            <div class="col-span-1 mr-2">
                <form action="{{ route('fridges.searchproduct') }}" method="get" class="flex flex-row">
                    <input
            ...
            <form action="{{ route('fridges.index') }}" method="post" class="col-span-1 col-start-4 mr-2 flex flex-row">
                @csrf
                @method('get')
            ...
            @if ($products->count())
                <table class="w-full divide-y divide-gray-100">
                    <thead  class="bg-blue-400 sticky">
                        <tr>
                            <th class="px-2 py-3 text-left text-lg rounded-tl-lg font-medium text-white">Product</th>
